...
- 将模型请求日志直接归属到用户
- 用户删除自助 key 后，历史模型请求日志仍然可以按用户查询
- 兼容旧日志时，可临时通过当前仍存在的 `token.owner_user_id` 反查
...
- 只返回当前用户自助 key 产生的请求日志
- 不返回其他用户、管理员 key、内部 token 的日志
- 支持分页参数 `p`、`per_page`
- 日志中可展示模型、分组、Key 名称、状态码、Token 用量、实际消费金额、请求 ID、请求时间

### `GET /user-api/logs/detail/:log_id`
